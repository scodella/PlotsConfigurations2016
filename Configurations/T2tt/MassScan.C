...
void MassScan(TString Type = "", TString Version = "", TString Signal = "") {
	
  TH2F *MassLimit, *hXsec_exp, *hR_exp, *hR_exp_up, *hR_exp_do;
  ...
  if (Type=="Do") CentralQuantile = 0.84;
  if (Type=="Up") CentralQuantile = 0.16;
  if (Type!="Blind") Type = "";
  ...
  while (InFile) {
    ...
    if (!MassPoint.Contains("#") && MassPoint!="") {
      ...
      TString RootFileName = "./Datacards" + Version + "/MassPoint" + MassPoint + "/datacardFinal" + Type + ".root";
      //RootFileName.ReplaceAll("2tt", "tt");
      TFile *RootFile = TFile::Open(RootFileName, "read");

      if (Signal=="") MassPoint.ReplaceAll("tt_isr", ""); 	     
      TString StopMass = MassPoint;
      StopMass.Replace(0, StopMass.First("_")+1, "");
      if (!Signal.Contains("Chi")) StopMass.Replace(0, StopMass.First("_")+1, "");
      ...
      if (iStopMass%5==0.) 
        GetStopCrossSection(1.*iStopMass);
      else {
          
          float StopMass1 = 5.*int(1.*iStopMass/5);
          GetStopCrossSection(StopMass1);
          float CrossSection1 = CrossSection, CrossSection1Uncertainty = CrossSectionUncertainty;
          
          float StopMass2 = 5.*(int(1.*iStopMass/5)+1);
          GetStopCrossSection(StopMass2);
          float CrossSection2 = CrossSection, CrossSection2Uncertainty = CrossSectionUncertainty;
          
          CrossSection = CrossSection1 + (1.*iStopMass-StopMass1)*(CrossSection2 - CrossSection1)/5.;
          CrossSectionUncertainty = CrossSection1Uncertainty + (1.*iStopMass-StopMass1)*(CrossSection2Uncertainty - CrossSection1Uncertainty)/5.;
          
      }

      if (CrossSection==-1) 
	cout << "Warning: cross section error for stop mass " << iStopMass << endl;	
	...
	if (_quantileExpected==CentralQuantile && CentralQuantile!=0.16 && CentralQuantile!=0.84) {
	  ...
	  if (CentralQuantile==0.16) MassLimit->Fill(iStopMass, iXMass, _limit);
        } else if (_quantileExpected>0.83 && _quantileExpected<0.85) { 	
	  hR_exp_do->Fill(iStopMass, iXMass, _limit);
	  if (CentralQuantile==0.84) MassLimit->Fill(iStopMass, iXMass, _limit);
        }
